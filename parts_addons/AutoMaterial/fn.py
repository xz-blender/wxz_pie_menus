...
def get_single_pixel_color_from_image(img):
    ...
    rgb = (img.pixels[index], img.pixels[index + 1], img.pixels[index + 2], 1.0)
        ...
        if scene.mat_change_multiple:
            for slot in ob.material_slots:
                ...
                if mat and mat not in matlist:
                    ...
                    if scene.only_unnamed and not mat.name.startswith("Material"):
                        continue
                    matlist.append(mat)
        else:
...
def match_color_node_from_viewport(variables={}):
    ...
    for i, mat in enumerate(matlist):
        ...
        if color:
            ...
            if not node:
                report(
                    'cant find a node to set color in the node tree... (this try only with the first node connected to "Surface")',
                    self=self,
                    mode="ERROR",
                )
                continue
