...
class AMT_OT_clean_gp_material_stack(bpy.types.Operator):
    ...
    use_clean_mats: bpy.props.BoolProperty(
    ...
    skip_different_materials: bpy.props.BoolProperty(
        name="Skip Different Material",
        description="Will not touch duplication if color settings are different (and show infos about skipped materials)",
        default=True,
    )

    use_fuses_mats: bpy.props.BoolProperty(
        name="Fuse Materials Slots",
        description="Fuse materials slots when multiple uses same materials",
        default=True,
    )

    remove_empty_slots: bpy.props.BoolProperty(
        name="Remove Empty Slots",
    ...
    def clean_mats_duplication(self, ob):
        ...
        if diff_ct:
            return (
                "INFO",
                f"{diff_ct} mat skipped >> same name but different color settings!",
            )
    ...
    def execute(self, context):
        ...
        if (
            not self.use_clean_mats
            and not self.use_fuses_mats
            and not self.remove_empty_slots
        ):
...
def material_gp_clean_menu(self, context):
    ...
    layout = self.layout
    layout.operator(
        "materials.clean_gp_material_stack",
