...
class AM_OT_auto_name_material(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        rename_mat(viewport=self.viewport, self=self, color_dic=color_dic, context=context)
        return {"FINISHED"}
...
def register():
    bpy.types.Scene.mat_change_multiple = bpy.props.BoolProperty(
        name="Affect All Slots",
        default=True,
        description="Affect all material slots (skip already named materials if option is active in addon prefs)\nelse only active slot",
        options={"HIDDEN"},
    )

    bpy.types.Scene.only_unnamed = bpy.props.BoolProperty(
        name="仅重命名未命名材质 (使用多次重命名时)",
        description="在 “重命名所有材质槽” 模式中，仅重命名未命名的材料 (以 “Material” 开始)",
        default=False,
    )  # type: ignore
    for cls in classes:
...
def unregister():
    ...
    del bpy.types.Scene.only_unnamed
