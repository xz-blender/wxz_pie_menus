...
class NB_RotationOffset_Operator(bpy.types.Operator):
    bl_idname = "nb_frame.rotation_offset"
    bl_label = "牛逼旋转错帧"
    bl_description = "点他牛旋转错帧"
...
#                    i.show_render = False
        ...
        def xuanZHCuoZhen(Abone,gensuiAmp =1):
             #####
           ...
           ###### 
            ...
            cons =emp.constraints.new('COPY_ROTATION')#空物体添加个旋转约束
            ...
            bonecons =Abone.constraints.new('COPY_ROTATION')#给骨骼添加一个复制xuanzhuan约束
            bonecons.target = emp
...
#############################vel########vel########vel################################# 
        ...
        for i in selboneS:            
            xuanZHCuoZhen(i,bpy.context.scene.Prop_nb_666.cInfluence)   
        for i in selboneS: 
