...
def main1(obj, context, operator, square, snapToClosest):
    ...
    for island in islands:
        ...
        if (
            targetFace == None
            or targetFace not in island
            or len(islands) > 1
            or targetFace.select == False
            or len(targetFace.verts) != 4
        ):
    ...
    for key in vertsDict:
        ...
        for f in bm.faces:
            for l in f.loops:
                ...
                if luv.select == False:
                    hyp = hypot(
                        vertsDict[(key[0], key[1])][0].uv.x - luv.uv.x, vertsDict[(key[0], key[1])][0].uv.y - luv.uv.y
                    )
...
def register():
    ...
    if wm.keyconfigs.addon:
        ...
        kmi = km.keymap_items.new(
            UV_PT_UvSquaresByShape.bl_idname,
            shortcut["key"],
            "PRESS",
            alt=shortcut["alt"],
            ctrl=shortcut["ctrl"],
