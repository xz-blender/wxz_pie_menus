...
class Mirror(bpy.types.Operator):
    bl_idname = "pie.m4mirror"
    bl_label = "M4A1: Mirror"
    bl_options = {"REGISTER", "UNDO"}

    ob_mirror: BoolProperty(name="Object Mirror", default=False)  # type: ignore
    flick: BoolProperty(name="Flick", default=True)  # type: ignore
    remove: BoolProperty(name="Remove", default=False)  # type: ignore
    axis: EnumProperty(name="Axis", items=axis_items, default="X")  # type: ignore
    use_x: BoolProperty(name="X", default=True)  # type: ignore
    use_y: BoolProperty(name="Y", default=False)  # type: ignore
    use_z: BoolProperty(name="Z", default=False)  # type: ignore
    bisect_x: BoolProperty(name="Bisect", default=False)  # type: ignore
    bisect_y: BoolProperty(name="Bisect", default=False)  # type: ignore
    bisect_z: BoolProperty(name="Bisect", default=False)  # type: ignore
    flip_x: BoolProperty(name="Flip", default=False)  # type: ignore
    flip_y: BoolProperty(name="Flip", default=False)  # type: ignore
    flip_z: BoolProperty(name="Flip", default=False)  # type: ignore
    DM_mirror_u: BoolProperty(name="U", default=True)  # type: ignore
    DM_mirror_v: BoolProperty(name="V", default=False)  # type: ignore
    cursor: BoolProperty(name="Mirror across Cursor", default=False)  # type: ignore
    use_misalign: BoolProperty(name="Use Mislinged Object Removal", default=False)  # type: ignore
    use_existing_cursor: BoolProperty(name="Use existing Cursor Empty", default=False)  # type: ignore
    passthrough = None
    ...
    # def __init__(self):
    #     self.cursor = False  # 或者设置为其他合适的默认值
    #     self.cmx = None
    #     self.mx = None
    def draw(self, context):
        layout = self.layout
        ...
        row = layout.row()
        # row.prop(self, "ob_mirror",text="不添加修改器" ,toggle=True)
        ...
        if (
            ...
            and self.active in context.selected_objects
            and not self.cursor
        ):
            row = column.row(align=True)
            r = row.row()
    ...
    def draw_HUD(self, context):
        if not self.passthrough:
            ...
            if self.remove and self.misaligned and self.use_misalign:
                name = (
                    "Cursor Empty"
                    if self.use_misalign and self.mirror_obj.type == "EMPTY"
                    else self.mirror_obj.name if self.use_misalign else "None"
                )
                alpha = 1 if self.use_misalign else 0.3
                color = (
                    blue
                    if self.use_misalign and self.mirror_obj.type == "EMPTY"
                    else yellow if self.use_misalign else white
                )

                draw_label(
                    context,
                    title=name,
                    coords=(self.init_mouse[0], self.init_mouse[1] - self.flick_distance + (30 * self.scale)),
                    center=True,
                    color=color,
                    alpha=alpha,
                )
                ...
                title, color = (
                    ("New Cursor", green)
                    if self.cursor and not self.use_existing_cursor
                    else ("Existing Cursor", blue) if self.cursor else (self.active.name, yellow)
                )
                draw_label(
                    context,
                    title=title,
            ...
            title = (
                self.flick_direction.split("_")[1] if self.remove else self.flick_direction.replace("_", " ").title()
            )
            draw_label(
                context,
                title=title,
        ...
        if event.type in events:
            if self.passthrough:
                ...
                if self.mirror_obj and self.mirror_obj.type == "EMPTY":
                    ...
                    self.cursor_empty_zoom = get_zoom_factor(
                        context, depth_location=loc, scale=10, ignore_obj_scale=True
                    )
            ...
            if event.type == "MOUSEMOVE":
                ...
                if self.flick_vector.length:
                    self.flick_direction = get_flick_direction(
                        context, self.init_mouse_3d, self.flick_vector, self.axes
                    )
                ...
                event.type in {"C", "E", "A", "X", "D", "R", "Q", "WHEELDOWNMOUSE", "WHEELUPMOUSE", "ONE", "TWO"}
                ...
                if event.type in {"X", "D", "R"}:
                    ...
                    self.ob_mirror = not self.ob_mirror
                    self.active.select_set(True)
                    ...
                    if event.type in ["WHEELDOWNMOUSE", "WHEELUPMOUSE", "ONE", "TWO"] and event.value == "PRESS":
                        if self.use_misalign:
                            if event.type in ["WHEELDOWNMOUSE", "ONE"]:
                                self.mirror_obj = step_list(
                                    self.mirror_obj, self.misaligned["sorted_objects"], step=-1, loop=True
                                )
    ...
    def invoke(self, context, event):
        ...
        if self.flick:
            ...
            self.scale = context.preferences.system.ui_scale * modal_hud_scale
            self.flick_distance = mirror_flick_distance * self.scale
            ...
            # print(f"cursor: {self.cursor}, cmx: {self.cmx}, mx: {self.mx}")
    ...
    def get_matching_cursor_empty(self, context):
        ...
        matching_empties = [
            ...
            if obj.type == "EMPTY" and compare_matrix(obj.matrix_world, self.cmx, precision=5)
        ]
    ...
    def mirror(self, context, active, sel):
        if self.ob_mirror:
            ...
            bpy.ops.transform.mirror(constraint_axis=(self.use_x, self.use_y, self.use_z))
            ...
            if self.cursor:
                if self.flick and self.cursor_empty and self.use_existing_cursor:
                    ...
                    empty.empty_display_size = (
                        context.scene.cursor.location - sel[0].matrix_world.to_translation()
                    ).length / 10
...
class Unmirror(bpy.types.Operator):
    ...
    def poll(cls, context):
        mirror_meshes = [
            ...
            if obj.type == "MESH" and any(mod.type == "MIRROR" for mod in obj.modifiers)
        ]
        if mirror_meshes:
            return True
        ...
        mirror_gpencils = [
            obj
            for obj in context.selected_objects
            if obj.type == "GPENCIL" and any(mod.type == "GP_MIRROR" for mod in obj.grease_pencil_modifiers)
        ]
        if mirror_gpencils:
            return True
