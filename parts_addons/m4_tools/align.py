...
class AlignEditMesh(bpy.types.Operator):
    ...
    mode: EnumProperty(name="Mode", items=align_mode_items, default="VIEW")  # type: ignore
    type: EnumProperty(
        name="Type", items=align_type_items, description="Align to Min or Max, Average, Zero or Cursor", default="MIN"
    )  # type: ignore
    axis: EnumProperty(name="Axis", items=axis_items, description="Align on the X, Y or Z Axis", default="X")  # type: ignore
    direction: EnumProperty(name="Direction", items=align_direction_items, default="LEFT")  # type: ignore
    space: EnumProperty(
        name="Space", items=align_space_items, description="Align in Local, World or Cursor Space", default="LOCAL"
    )  # type: ignore
    align_each: BoolProperty(name="Align Each Island independently", default=False)  # type: ignore
    draw_each: BoolProperty()  # type: ignore
    ...
    def invoke(self, context, event):
        ...
        if self.mode == "VIEW":
            ...
            if self.type in ["ZERO", "AVERAGE", "CURSOR"] and self.direction in ["HORIZONTAL", "VERTICAL"]:
                ...
                popup_message(
                    "You can't combine {} with {}!".format(self.type, self.direction),
                    title="Invalid Property Combination",
                )
                return {"CANCELLED"}
    ...
    def align(self, context, type, axis, space):
        ...
        if tuple(bpy.context.scene.tool_settings.mesh_select_mode) == (True, False, False) or type in [
            "ZERO",
            "CURSOR",
        ]:
    ...
    def get_matrix(self, context):
        mx = (
            ...
            if self.space == "LOCAL"
            else context.scene.cursor.matrix if self.space == "CURSOR" else Matrix()
        )
        return mx
...
class CenterEditMesh(bpy.types.Operator):
    ...
    direction: EnumProperty(name="Axis", items=align_direction_items, default="HORIZONTAL")  # type: ignore
    space: EnumProperty(name="Space", items=align_space_items, default="LOCAL")  # type: ignore
...
class AlignObjectToEdge(bpy.types.Operator):
    ...
    def get_vectors_from_alignment_edges(self, obj, target):
        ...
        v_obj = (
            (obj.matrix_world.to_3x3() @ Vector(edges[0].verts[0].co - edges[0].verts[1].co)).normalized()
            if len(edges) == 1
            else None
        )
        ...
        v_target = (
            (target.matrix_world.to_3x3() @ Vector(edges[0].verts[0].co - edges[0].verts[1].co)).normalized()
            if len(edges) == 1
            else None
        )
