...
def draw_distribution_x(layout, x):
    op = layout.operator(
        AlignObject.bl_idname, text=set_text("Distribution"), icon_value=get_icon("Align_Distribution_X")
    )
...
def draw_distribution_y(layout, y):
    op = layout.operator(
        AlignObject.bl_idname, text=set_text("Distribution"), icon_value=get_icon("Align_Distribution_Y")
    )
    op.align_mode = "DISTRIBUTION"
...
def draw_ground(layout):
    op = layout.operator(AlignObject.bl_idname, text=set_text("Ground"), icon="IMPORT")
...
def draw_fall(layout):
    op = layout.operator(AlignObject.bl_idname, text=set_text("Fall"), icon="AXIS_TOP")
    op.align_mode = "GROUND"
    op.ground_down_mode = "ALL"
...
def draw_world(layout):
    op = layout.operator(AlignObject.bl_idname, text=set_text("World Original"), icon="OBJECT_ORIGIN")
    op.align_mode = "ORIGINAL"
    op.align_location = True


def draw_active(layout):
    op = layout.operator(AlignObject.bl_idname, text=set_text("Active"), icon="RESTRICT_SELECT_OFF")
...
def draw_cursor(layout):
    op = layout.operator(AlignObject.bl_idname, text=set_text("Cursor"), icon="PIVOT_CURSOR")
