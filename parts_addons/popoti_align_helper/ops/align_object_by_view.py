...
class ObjectAlignByView(Operator):
    ...
    def draw_nine_square_box(layout: bpy.types.UILayout, show_text=False, ops=None):
        ...
        args = dict(show_text=show_text, ops=ops)
    ...
    def _item_(layout: bpy.types.Operator, identifier, show_text, ops):
        ...
        if ops:
            ...
            o = layout.operator("wm.context_set_enum", icon_value=get_icon(identifier), text=text)
            o.data_path = "ops.align_mode"
            o.value = identifier
        else:
            layout.operator(ObjectAlignByView.bl_idname, icon_value=get_icon(identifier), text=text).align_mode = (
                identifier
            )
