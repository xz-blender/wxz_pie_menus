...
class Mesh_UVScaleOperator(Operator):
    ...
    scale: FloatProperty(default=1.0, min=-100.0, max=100.0)
    @classmethod
    def poll(cls, context):
        return (context.active_object is not None and
                context.active_object.type == 'MESH' and
    ...
    def execute(self, context):
        ...
        for face in bm.faces:
            if face.select:
                for loop in face.loops:
                    loop_uv = loop[uv_layer]
                    loop_uv.uv = ((loop_uv.uv[0] - uv_center[0]) * factor + uv_center[0], 
...
class PIE_ScaleUVOperator(bpy.types.Operator):
    ...
    scale_u: bpy.props.FloatProperty(name="Scale U",description="Scale U coordinates",default=1.0)
    scale_v: bpy.props.FloatProperty(name="Scale V",description="Scale V coordinates",default=1.0)
...
def register_keymaps():
    ...
    km = addon.keymaps.new(name='Mesh')
    kmi = km.keymap_items.new(
    idname='wm.call_menu_pie', type="S", value="CLICK_DRAG",ctrl = True,alt = True)
